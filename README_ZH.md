...
### 阶段 5： Instalación del Escritorio XFCE, Servidores Gráficos y Herramientas
安装 XFCE4 图形环境及其核心组件、显示服务器 (`TigerVNC` y original `termux-x11-nightly` 包)，以及音频系统 y herramientas de desarrollo nativas:

```bash
# Entorno XFCE ultraligero y aplicaciones básicas (agrupado para optimización)
pkg install xfce4 xfce4-goodies thunar htop xfce4-places-plugin picom geany qgit audacity mtpaint -y 

# Gráficoservicios y herramientas ADB
pkg install tigervnc android-tools -y
pkg install termux-x11-nightly -y  # Servidor X11 nativo de alta fluidez

# Audio y configuración posterior
...
# Forzar renderizado por software y establecer variables de entorno (sincronizado con vnc-on)
...
**Importante:** El renderizado está diferenciado por aplicación:

* **Code-OSS (VS Code):** Renderizado **por software** (`LIBGL_ALWAYS_SOFTWARE=1`) para evitar *glitches* de Electron. No se recomienda usar aceleración GPU con Code-OSS, ya que su arquitectura Electra se beneficia del renderizado CPU.
* **Firefox y Godot Engine:** Utilizan la **GPU bajo demanda** (`GALLIUM_DRIVER=virpipe`) cuando se requiere procesamiento 3D. Estas aplicaciones están optimizadas para cambiar entre renderizado software y GPU según la necesidad.

Para abrirlos directamente desde el Menú Whisker o el Panel inferior:

1. Haz clic derecho sobre la aplicación en el Menú Whisker o en el Panel y selecciona **Editar aplicación** (o *Propiedades*).
2. En la casilla **Comando**, usa el prefijo apropiado:
...
> **Nota:** No es necesario (ni recomendable) aplicar ajustes GPU a herramientas ligeras del sistema como el administrador de archivos o el gestor de tareas. El entorno XFCE siempre usa renderizado por software para estabilidad.
...
# Arranque limpio y nativo de XFCE
...
# 2. 启动显示服务器 en modo Vulkan y limpieza profunda (Consistencia con X11)
vgl q 2>/dev/null
pkill -9 -f virgl 2>/dev/null
vgl angle=vulkan &
sleep 2
