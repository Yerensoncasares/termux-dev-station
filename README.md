...
## 🕹️ Scripts de Automatización (`up.sh`, `on.sh`, `off.sh`, `ia.sh`)
...
### 3. Script de Apagado Limpio (`off.sh`)
Detiene los demonios gráficos, servidores X11/VirGL, libera PulseAudio y limpia sockets temporales.
```bash
#!/data/data/com.termux/files/usr/bin/bash
...
### 4. Script de Inteligencia Artificial / OpenCode (`ia.sh`)
...
> **Nota:** Para crear y activar cualquiera de estos scripts, puedes crearlos con `nano <nombre>.sh`, pegar el contenido, guardarlo y ejecutar:
> ```bash
> chmod +x nombre.sh
> ```
