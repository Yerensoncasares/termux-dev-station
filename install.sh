...
if ! ping -c 1 github.com &>/dev/null && ! ping -c 1 8.8.8.8 &>/dev/null; then
    echo -e "${RED}[ERROR CRÍTICO] Sin conexión a internet. Se requiere acceso a la red para la instalación.${NC}"
    echo -e "${RED}Por favor verifica tu conexión y vuelve a intentar.${NC}"
    exit 1
fi
echo -e "${GREEN}[✓] Conexión a internet verificada correctamente.${NC}"
...
if [[ "$choice_ai" =~ ^[sS]$ ]]; then
    ...
    if [ ! -f "$PREFIX/bin/opencode" ]; then
        echo -e "${BLUE}[*] Obteniendo la última versión de OpenCode desde GitHub...${NC}"
        LATEST_OPCODE=$(curl -s https://api.github.com/repos/guysoft/opencode-termux/releases/latest | grep "browser_download_url.*aarch64.zip" | cut -d '"' -f 4 || true)
        if [ -z "$LATEST_OPCODE" ]; then
            LATEST_OPCODE="https://github.com/guysoft/opencode-termux/releases/latest/download/opencode-1.17.9-android-aarch64.zip"
        fi
...
pulseaudio --start --exit-idle-time=-1 2>/dev/null
[ -r $HOME/.Xresources ] && xrdb $HOME/.Xresources
dbus-launch --exit-with-session DE_START_PLACEHOLDER
EOF
    if [ "$choice_de" == "1" ]; then
        sed -i "s/QT_ENV_PLACEHOLDER/export QT_QPA_PLATFORM=xcb/g" on.sh
    else
        sed -i "/QT_ENV_PLACEHOLDER/d" on.sh
    fi
    sed -i "s/DE_START_PLACEHOLDER/$DE_START/g" on.sh
    chmod +x on.sh
fi
