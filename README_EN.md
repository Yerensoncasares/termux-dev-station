...
```bash
# Ultra-lightweight XFCE environment and file manager (grouped for optimization)
...
# 4. Install automatic launchers and desktop configuration
...
**Important:** Rendering is differentiated by application:

* **Code-OSS (VS Code):** **Software rendering** (`LIBGL_ALWAYS_SOFTWARE=1`) to avoid Electron glitches. It is not recommended to use GPU acceleration with Code-OSS, as its Electron architecture benefits from CPU rendering.
* **Firefox and Godot Engine:** Use **on-demand GPU** (`GALLIUM_DRIVER=virpipe`) when 3D processing is required. These applications are optimized to switch between software and GPU rendering as needed.

To open them directly from the Whisker Menu or Bottom Panel:

1. Right-click on the application in the Whisker Menu or Panel and select **Edit Application** (or *Properties*).
...
> **Note:** It is not necessary (nor recommended) to apply GPU adjustments to lightweight system tools like the file manager or task manager. The XFCE environment always uses software rendering for stability.
